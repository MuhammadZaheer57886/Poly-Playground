...
class _UserProfileState extends State<UserProfile> {
  ...
  void initState() {
    ...
    getUserData(widget.userId).then((value) {
      if (value != null) {
        ...
        checkFriendRequest(userData.uid).then((value) => {
              if (value != null)
                {
                  setState(() => friendRequest = value),
                  if (value.senderId == Store().uid)
                    {
                      setState(() {
                        isLiked = true;
                      })
                ...
                {
                  checkIsFriend(userData.uid).then((value) => {
                        if (value)
                          {
                            setState(() {
                              isLiked = true;
  ...
  void unLike(UserDataModel user) async {
    if (friendRequest != null) {
      await (friendRequest!.senderId == user.uid
  ...
  void handelLike(UserDataModel userData) async {
    if (friendRequest == null) {
      await makeFriendRequest(userData);
    } else {
